...
class Game:
    ...
    def new(self):
        ...
        for tile_object in self.map.tmxdata.objects:
            ...
            if tile_object.name=="hill":
                Hill(self,tile_object.x,tile_object.y,tile_object.width,tile_object.height)
            if tile_object.name.startswith('wall'):
                     if tile_object.name.endswith("_ice"):
                        Obstacle(self,tile_object.x,tile_object.y,tile_object.width,tile_object.height)
                     else:
                          Obstacle(self,tile_object.x,tile_object.y,tile_object.width,tile_object.height)
            if tile_object.name.startswith('destructable'):
                    list=tile_object.name.split('_')
                    type=list[1]
                    key=int(list[2])
                    dir=list[3]
                    Destructable(self,tile_object.x,tile_object.y,type,key,dir)
            ...
            if tile_object.name=="platform":
                Platform(self,tile_object.x,tile_object.y,tile_object.width,tile_object.height)
            if tile_object.name.startswith("leveler"):
                list=tile_object.name.split("_")
                type=list[1]
                key=int(list[2])
                Leveler(self,tile_object.x,tile_object.y,type,key)
                


            if tile_object.name.startswith("hidden_item"):
                print(tile_object.name[-4:])
                print(self.mapname)
           #     Item(self,tile_object.x,tile_object.y,int(tile_object.name[-4:]),False,True)
                Item(self,tile_object.x,tile_object.y,0,is_hidden=True,rand_rarity='usual')
            ...
            if tile_object.name.startswith("trap"):           
                     ...
                     try:
                         ...
                         key=0
                     Trap(self,tile_object.x,tile_object.y,tile_object.width,tile_object.height,type,key)
        ...
        # обновление кадра
           ...
           if not self.player_1.inv.active and not self.player_1.stats.active:    
                  ...
                  for sprite in self.all_sprites:
                    if not isinstance(sprite,Attack):
                        vector=vec(self.player_1.rect.x-sprite.rect.x,self.player_1.rect.y-sprite.rect.y)
                        if vector.length()<3000:
                              sprite.update()
