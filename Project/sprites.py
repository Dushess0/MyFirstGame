...
class Player(pg.sprite.Sprite):
    ...
    def colliding_with_hills(self,dir):
        if dir == 'x':
            hits = pg.sprite.spritecollide(self, self.game.hills, False)
            if hits:
                if self.vel.x > 0:
    ...
    def jump(self):
            ...
            group_choose(self.game.walls)
            group_choose(self.game.hills)
            group_choose(self.game.platforms)
    ...
    def update(self):
        ...
        self.rect.y = self.pos.y
        self.collide('y')
        self.collide_with_hills('x')
        self.collide_with_hills('y')
...
class Destructable(pg.sprite.Sprite):
     def __init__(self, game, x, y,type,key,dir):
        ...
        if self.type=="bars":
            print("recognized")
            self.image=pg.image.load(bars_metal)
        if self.type=="spikes":
            print("recognized")
            self.image=pg.image.load(bars_spikes)
     ...
     def must_move(self):
         if self.dir=="right" or self.dir=="left":
            if abs(self.rect.x-self.start.x)<64 :
                return True
         else:
            if  abs(self.rect.y-self.start.y)<64:
                 return True
     def move(self,dir):
        if self.must_move():
          if dir=="left":
...
class Trap(pg.sprite.Sprite):
    def __init__(self,game,x,y,w,h,type,trigger=0):
        ...
        if type=="hidden spike":
            self.damage=20
            self.reload=1
            self.type_of_damage="physical"
            self.self.orig= pg.image.load(spike_image)
            rect=self.orig.get_rect()
            self.image= pg.Surface([rect.width,rect.height], pg.SRCALPHA,32)
            self.active=False
...
class Leveler(pg.sprite.Sprite):
    def __init__(self, game, x, y,type,key):
        self.groups = game.buttons,game.all_sprites
        pg.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        self.image=pg.image.load(img_leveler)
        self.key=key
        self.type=type
...
class Boss(Enemy):
     def __init__(self,game,x,y,name,level):
         #базовое
        self.groups=game.enemies,game.all_sprites
        pg.sprite.Sprite.__init__(self,self.groups)
        self.rot='right'
        ...
        self.state='right'
        self.rot=vec(1,0)
        with open(boss_file) as file:
            data=json.load(file)
        for dictionary in data:
            if dictionary["name"]==self.name:
